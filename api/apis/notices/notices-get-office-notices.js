...
const operationMetadata = {
	group:       `notices`,
	name:        `get-office-notices`,
	path:        `/api/v1/offices/:office_id/notices`,
	method:      `GET`,
	description: `Returns notices associated with office identified by supplied office_id.`,
	parameters:  {
		office_id: { description: `ID of the office to find associated notices`, type: `query`, optional: false  },
	...
	scopes:             [ `read:notices`, `read:offices`  ],
	modelName:          `Notice`,
	...
	Object.assign(operationMetadata, {
		async action (req, resp, next) {
			...
			try {
				...
				const result = await api.notices.getAll({ where: whereQuery, skip, limit });

				// DEBUG: get-office-notices result
				// console.debug(`🦠  get-office-notices result: ${JSON.stringify(result, null, 2)}`);
