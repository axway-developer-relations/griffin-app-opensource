...
const operationMetadata = {
	...
	path:        `/api/v1/notices`,
	method:      `GET`,
	description: `Returns all notices from the system.`,
	parameters:  {
	...
	Object.assign(operationMetadata, {
		async action (req, resp, next) {
			...
			try {
				...
				if (object_id) {
					result = await api.notices.getByObjectId(object_id);
					...
					result = await api.notices.getAll({ where, limit, skip });
				}

				// DEBUG: get-notices result
				// console.debug(`🦠  get-notices result: ${JSON.stringify(result, null, 2)}`);
