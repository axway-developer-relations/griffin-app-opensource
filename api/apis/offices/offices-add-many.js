...
const operationMetadata = {
	group:       `offices`,
	name:        `post-offices`,
	path:        `/api/v1/offices/bulk`,
	method:      `POST`,
	description: `Creates one or more offices in the system.`,
	parameters:  {
		name:              { description: `Name of office`, type: `body`, optional: true },
		description:       { description: `Description of office`, type: `body`, optional: true },
		timezone:          { description: `Timezone where this office is located`, type: `body`, optional: true },
		...
		max_capacity:      { description: `Maximum capacity for this office`, type: `body`, optional: true },
	...
	Object.assign(operationMetadata, {
		async action (req, resp, next) {
			...
			try {
				...
				const results = [];
				const meta = [];
				for (const office of req.body) {

					const { name, description, timezone, address, address2, city, state, country, max_capacity, location, allow_checkins, notice_sources, entity_created_at, entity_updated_at, tags, postalcode  } = office;
					...
					const result = await api.offices.add(entity);
