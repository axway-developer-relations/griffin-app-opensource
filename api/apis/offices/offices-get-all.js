...
const operationMetadata = {
	...
	path:        `/api/v1/offices`,
	method:      `GET`,
	description: `Returns all offices from the system.`,
	parameters:  {
	...
	Object.assign(operationMetadata, {
		async action (req, resp, next) {
			...
			try {
				...
				if (object_id) {
					result = await api.offices.getByObjectId(object_id);
					...
					result = await api.offices.getAll({ where, limit, skip });
				}

				// DEBUG: get-offices result
				// console.debug(`🦠  get-offices result: ${JSON.stringify(result, null, 2)}`);
