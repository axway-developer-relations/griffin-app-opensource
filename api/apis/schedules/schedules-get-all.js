	...
	Object.assign(operationMetadata, {
		async action (req, resp, next) {
			...
			try {
				...
				if (object_id) {
					result = await api.schedules.getByObjectId(object_id);
					...
					result = await api.schedules.getAll({ where, limit, skip });
				}

				// DEBUG: get-schedules result
				// console.debug(`🦠  get-schedules result: ${JSON.stringify(result, null, 2)}`);
