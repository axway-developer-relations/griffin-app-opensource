...
const operationMetadata = {
	...
	method:      `GET`,
	description: `Returns all capacities from the system.`,
	parameters:  {
	...
	Object.assign(operationMetadata, {
		async action (req, resp, next) {
			...
			try {
				...
				if (object_id) {
					result = await api.capacities.getByObjectId(object_id);
					...
					result = await api.capacities.getAll({ where, limit, skip });
				}

				// DEBUG: get-capacities result
				// console.debug(`🦠  get-capacities result: ${JSON.stringify(result, null, 2)}`);
