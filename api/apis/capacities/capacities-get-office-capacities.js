...
const operationMetadata = {
	group:       `capacities`,
	name:        `get-office-capacities`,
	path:        `/api/v1/offices/:office_id/capacities`,
	method:      `GET`,
	description: `Returns capacities associated with office identified by supplied office_id.`,
	parameters:  {
		office_id: { description: `ID of the office to find associated capacities`, type: `query`, optional: false  },
	...
	scopes:             [ `read:capacities`, `read:offices`  ],
	modelName:          `Capacity`,
	...
	Object.assign(operationMetadata, {
		async action (req, resp, next) {
			...
			try {
				...
				const result = await api.capacities.getAll({ where: whereQuery, skip, limit });

				// DEBUG: get-office-capacities result
				// console.debug(`🦠  get-office-capacities result: ${JSON.stringify(result, null, 2)}`);
