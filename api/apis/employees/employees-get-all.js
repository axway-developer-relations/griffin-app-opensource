	...
	Object.assign(operationMetadata, {
		async action (req, resp, next) {
			...
			try {
				...
				if (object_id) {
					result = await api.employees.getByObjectId(object_id);
					...
					result = await api.employees.getAll({ where, limit, skip });
				}

				// DEBUG: get-employees result
				// console.debug(`🦠  get-employees result: ${JSON.stringify(result, null, 2)}`);
