...
const operationMetadata = {
	...
	method:      `GET`,
	description: `Returns all checkins from the system.`,
	parameters:  {
	...
	Object.assign(operationMetadata, {
		async action (req, resp, next) {
			...
			try {
				...
				if (object_id) {
					result = await api.checkins.getByObjectId(object_id);
					...
					result = await api.checkins.getAll({ where, limit, skip });
				}

				// DEBUG: get-checkins result
				// console.debug(`🦠  get-checkins result: ${JSON.stringify(result, null, 2)}`);
